  ...
  const handleWorkFilter = (item) => {
    ...
    setTimeout(() => {
      ...
      if (item === 'All') {
        ...
        setFilterWork(work.filter((works) => works.tags.includes(item)))
      }
    }, 500)
  ...
  return (
    <>
      ...
      <div className='app__work-filter'>
          {['Web App', 'Mobile App', 'React', 'Other'].map((item, index) => (
          <div
            key={index}
            onClick={() => handleWorkFilter(item)}
            className={`app__work-filter-item app__flex p-text${item === activeFilter ? 'item-active' : ''}`}
          >
            {item}
          </div>
        ))}
